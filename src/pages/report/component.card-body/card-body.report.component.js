...
const ReportBody = ({ reports, category, finishQuiz, resetReport }) => {
  ...
  return (
    <CardBodyTrivia>
      ...
      <ResultsSumary tabIndex="0">
        <div>
          <div className="sumary__number" data-testid="report-rights">
            {reportDatas.rights}
          </div>
          <div className="sumary__text">Correct</div>
        </div>
        <div>
          <div className="sumary__number" data-testid="report-wrongs">
            {reportDatas.wrongs}
          </div>
      ...
      <ResultsDetails tabIndex="0">
        {Object.keys(reportDatas.difficulty).map((difficulty) => (
          <div
            key={difficulty}
            className="level__details"
            data-testid="report-level-details"
          >
            <h6 className="level__details--title">
              {difficulty[0].toUpperCase() + difficulty.substring(1)}
            </h6>
            <p
              className="level__details--info"
              data-testid={`report-${difficulty}-rights`}
            >
              Correct: {reportDatas.difficulty[difficulty].rights}
            </p>
            <p
              className="level__details--info"
              data-testid={`report-${difficulty}-wrongs`}
            >
